...
- **`asignarOrdenManual(preferida, otra, listaTareas)`** (v0.69.0): asigna `tarea_prioridad_manual` a un par — `preferida` el valor más bajo (gana), `otra` el siguiente; valores frescos, siempre crecientes (`1 + máximo existente`). La usa tanto "Versus" como reordenar a mano con ▲▼.
- **`motivoBloqueoOrdenManual(actual, vecina, categorias)`** (v0.69.0): por qué `actual` no puede cambiar de orden con `vecina` (▲▼ a mano) — `''` si sí puede. Bloquea si son cadena previa/próxima, o si no están empatadas en `compararEstructural` (mismo orden de niveles: holgura, categoría raíz, categoría directa, importancia); el texto devuelto nombra a `vecina` y qué la hace ganar, para que el usuario sepa qué campo tocar. La usan las ▲▼ de `views/tareas.view.js`, `views/tabla.view.js` y `views/gantt.view.js`.
...
- **`renderTarea(tarea, indice, activas, actualizarBarraSeleccion)`**: tarjeta con el borde izquierdo del color de la categoría (`--color-categoria`), etiqueta "⚠️ Vencida" y fondo rojizo si está vencida, los badges (incluida la persona asociada, 👤, desde la v0.66.0), el checklist con casillas que se tildan ahí mismo (`checklist-tarjeta.js`, persiste al tildar) y las acciones (▲▼ para reordenar a mano — v0.69.0, ver más abajo —, cambiar estado — solo `pendiente`/`completada` —, posponer, editar, duplicar, crearle previa/posterior, eliminar). "Editar" abre `abrirEdicionTarea`; completar usa `cumplirTarea`, volver a pendiente `reabrirTarea` y eliminar `eliminarTarea`. `indice`/`activas` (el orden global, aunque "Agrupar por categoría" esté activo) son opcionales: sin ellos (las completadas) no se muestran las ▲▼. En modo selección (v0.70.0) suma una casilla al principio de la tarjeta.
- **▲▼ reordenar a mano** (v0.69.0): comparan `tarea` contra `activas[indice - 1]`/`activas[indice + 1]` con `motivoBloqueoOrdenManual` (`tareas-logica.js`); si no se puede, el botón queda `disabled` con el motivo en el `title`. Si se puede, `asignarOrdenManual(actual, vecina, estado.tareas)` (▲) o `asignarOrdenManual(vecina, actual, estado.tareas)` (▼) y `persistirYNotificar()` (el redibujado sale solo, por la suscripción de `app.js`, mismo patrón que ▲▼ de Categorías).
...
- **Selección múltiple y edición masiva** (v0.70.0): "☑️ Seleccionar" alterna `modoSeleccion` (variable de módulo); activado, cada tarjeta de `activas` suma una casilla (`data-seleccionar`) ligada al `Set` de módulo `seleccionadas`. Tildarla solo actualiza `seleccionadas` y la barra de selección (`actualizarBarraSeleccion`, cuenta + habilita "✏️ Editar tareas seleccionadas") — **sin** redibujar toda la lista, para no perder el scroll. Al entrar a `renderVistaTareas` se filtra `seleccionadas` contra `estado.tareas` vigentes.
- **`abrirEdicionMasiva(tareas, alTerminar)`** (v0.70.0): `abrirDialogoFormulario` con `htmlFormularioEdicionMasiva()` — una fila por campo (`filaCampoMasivo`) con una casilla "Cambiar" que habilita el control real; sin tildar, ese campo no se toca. Campos: categoría, importancia, disfrute, meta, persona, ubicación (desplegables de `formulario-tarea.js`, sin la opción "＋ Crear nueva…", recortada con `sinOpcionNueva`), fecha límite (solo fecha, sin hora), duración, costo estimado y días hábiles (`htmlDiasHabiles`). Al guardar, `leerEdicionMasiva` arma `cambios` solo con los campos tildados y `aplicarCamposATarea(tarea, cambios)` (`formulario-tarea.js`, ya existía) los aplica a cada tarea elegida; sin ningún campo tildado, avisa y no hace nada. Quedan fuera a propósito: nombre/descripción, los enlaces depende-de/bloquea-a (regla 1 a 1, no se le puede asignar la misma previa a varias tareas) y marcar como completada (dispara efectos por tarea que no encajan con "aplicar lo mismo a todas").
...
- **`crearPanelVersus(contenedorVista)`**: panel toggleable (botón "⚔️ Versus") que ofrece de a un par de tareas empatadas (`construirClusteres`/`proximoParVersus`, sobre `esTareaAccionable`) para que el usuario elija cuál prefiere, o las omita; elegir llama `asignarOrdenManual` (`tareas-logica.js`, v0.69.0: antes repetía el cálculo acá, ahora es la misma función que usan las ▲▼). Ver `REGLAS_DE_PRIORIDAD.md` para el mecanismo completo (por qué "omitir" no asigna nada).
