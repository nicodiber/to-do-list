...
- **Reprogramado y programación automática al iniciar** (`reprogramarSiCorresponde()`): una sola vez, cuando los datos ya están listos (después de `inicializarAlmacenamiento()` o de conectar), se llama `reprogramarFechasSugeridasVencidas(estado.tareas)` (`tareas-logica.js`), `reubicarTareasSolapadas(estado)` (`programador.js`, v0.74.0), `programarTareasSinFecha(estado)` (`programador.js`) y `reprogramarTareaInmediataSiVencio(estado)` (`programador.js`, v0.74.0); si alguna afectó tareas, se persiste y se avisa con un `alert()` combinado que **nombra las tareas** (con su categoría si hace falta desambiguar, vía `caminoCategoria`) en vez de solo contarlas — junta en una sola lista las que quedaron sin hueco libre antes de su fecha límite (de las 3 funciones que pueden dejar alguna sin programar) y, aparte, las que quedaron con la sugerida después de su propia fecha límite tras un corrimiento en cascada (`avisoInconsistentes`, v0.76.0, ver `tareas-logica.js`). Ver `REGLAS_DE_PRIORIDAD.md`.
- **Chequeo periódico de la tarea inmediata** (v0.74.0): mientras la app sigue abierta, un `setInterval` de módulo (cada 90 s) vuelve a llamar `reprogramarTareaInmediataSiVencio(estado)` (con la misma guarda `datosListos && !soloLectura`); si reprogramó algo, persiste **sin** `alert()` (a diferencia del chequeo de `reprogramarSiCorresponde`, que sí avisa — repetir un `alert()` cada 90 s sería invasivo).
...
- **Verificación automática** (`verificar()`): al volver a la pestaña (`visibilitychange`), al recuperar red (`online`) y cada 5 minutos; solo actualiza la hora de "verificado" si no hay nada nuevo. Detecta además el reloj desfasado (>2 min) comparando la hora local con el `modifiedTime` devuelto en una subida. Cada corrida también invalida la caché de eventos de Calendar (`invalidarCacheEventos()` de `google-calendar.js`, v0.76.0) — las vistas que muestran eventos los vuelven a pedir en su próximo render, que ya dispara `notificar()`.
...
- **Constantes de UI**: `ESTADOS_TAREA`/`ETIQUETAS_ESTADO` (`bloqueada`/`pendiente`/`completada`), `PLAZOS_META`/`ETIQUETAS_PLAZO`, `UNIDADES_MANTENIMIENTO`/`ETIQUETAS_UNIDAD_MANTENIMIENTO`. `tarea_urgente` (booleano, v0.75.0 — antes `tarea_importancia` con `NIVELES_IMPORTANCIA`/`ETIQUETAS_IMPORTANCIA`/`ICONOS_IMPORTANCIA`, eliminadas) no tiene constantes propias: es un interruptor Sí/No.
...
- **`textoFechaHumana(fechaISO)`** (v0.74.0): fecha±hora de Tarea en lenguaje humano para fechas cercanas — "hoy", "mañana", "pasado mañana", "próximo lunes"…"próximo domingo" (hasta 8 días), y el formato de siempre (`formatearFechaOFechaHora`) para el resto (más lejos, o vencida). La usa `views/hoy.view.js` para "Sugerida"/"Límite".
- **`escaparHtml(texto)`**: sanitiza texto libre antes de insertarlo en `innerHTML`.
- **`conservarFoco(contenedor, redibujar)`** (v0.76.0): ejecuta `redibujar()` (que reemplaza HTML dentro de `contenedor`) conservando el foco — si el elemento enfocado está dentro de `contenedor` y tiene `id`, guarda esa `id` y la posición del cursor antes de redibujar, y si después de redibujar existe un elemento con la misma `id`, le devuelve el foco y la selección. La usan los buscadores de `views/tareas.view.js`, `views/tabla.view.js` y `views/gantt.view.js` (antes perdían el foco en cada letra escrita, porque `renderVistaX` redibuja todo el contenedor en cada evento `input`).
...
- **`reprogramarTareaConCascada(tarea, nuevaFechaSugeridaISO, listaTareas)`**: actualiza `tarea_fecha_sugerida` y, si había un valor previo, desplaza en cascada (mismo delta de tiempo, vía `desplazarFecha`) a las tareas que dependen de ella (`tarea_dependiente === tarea.tarea_id`). **Nunca toca `tarea_fecha_limite`** (v0.76.0 — antes la desplazaba también; solo el usuario puede cambiarla). Devuelve un array `inconsistentes` con las dependientes que, tras el desplazamiento, quedaron con la sugerida después de su propia fecha límite (sin tocar) — para avisar en vez de corregirlas sola; ver `avisoInconsistentes`.
- **`avisoInconsistentes(inconsistentes)`** (v0.76.0): texto del aviso para el array que devuelve `reprogramarTareaConCascada` (nombra las tareas afectadas), o `''` si está vacío. Lo llaman los ~10 puntos de la app que reprograman una tarea con dependientes (Hoy, Tareas, Agenda, "Revisar mi día", Gantt, edición de tarea, edición masiva, próximo contacto de Persona, programador automático).
...
- **`asignarOrdenManual(preferida, otra, listaTareas)`** (v0.69.0): asigna `tarea_prioridad_manual` a un par — `preferida` el valor más bajo (gana), `otra` el siguiente; valores frescos, siempre crecientes (`1 + máximo existente`). Pensada para "Versus": la ganadora salta al frente de todo el grupo empatado (a propósito, ahí se está decidiendo una preferencia definitiva). La sigue usando "Versus" y el ▲▼ de `views/gantt.view.js`.
- **`intercambiarAdyacentes(arriba, abajo, listaOrdenada, listaTareas, categorias)`** (v0.74.0): el ▲▼ de `views/tareas.view.js` y `views/tabla.view.js` (no Gantt) entre vecinas **no encadenadas**. A diferencia de `asignarOrdenManual`, no salta al frente de todo el grupo empatado — usarla ahí hacía que dos tareas todavía sin decidir "saltaran" por delante de otras tareas del mismo grupo que no tenían nada que ver con el click, dando un orden ilógico a simple vista (bug real reportado por el usuario). En cambio, encuentra el tramo contiguo de `listaOrdenada` (la lista tal como se muestra) que sigue empatado en `compararEstructural` y sin cadena con el par (`empatadasSinCadena`, privada) y le da a **todo ese tramo** un valor fresco y creciente, respetando el orden actual de todas salvo el par que se intercambia — así el resto del grupo no se mueve, y a partir de ahí un próximo ▲▼ dentro del mismo grupo es un intercambio simple.
- **`intercambiarCadena(nuevoPrimero, nuevoSegundo, listaTareas)`** (v0.75.0): el ▲▼ de `views/tareas.view.js` y `views/tabla.view.js` entre vecinas **encadenadas** (relación previa/próxima directa) — antes se bloqueaba, ahora la app reordena la cadena sola. El llamador garantiza que `nuevoSegundo` es hoy la previa de `nuevoPrimero`; invierte el tramo (P→segundo→primero→N pasa a P→primero→segundo→N), reasignando `tarea_dependiente` de las hasta 4 tareas involucradas y llamando `recalcularBloqueo` en cada una. Solo pointers, sin pasar por `evaluarEnlace` (permutación local de un tramo ya válido, no puede crear ciclos ni romper la regla 1 a 1) ni tocar fechas ni `tarea_desencadenante`.
- **`motivoBloqueoOrdenManual(actual, vecina, categorias)`** (v0.69.0): por qué `actual` no puede cambiar de orden con `vecina` (▲▼ a mano) — `''` si sí puede. Si son cadena previa/próxima directa, siempre `''` (v0.75.0 — antes bloqueaba; ver `intercambiarCadena`). Si no, bloquea cuando no están empatadas en `compararEstructural` (mismo orden de niveles: holgura, categoría raíz, categoría directa, `tarea_urgente`); el texto devuelto nombra a `vecina` y qué la hace ganar. La usan las ▲▼ de `views/tareas.view.js`, `views/tabla.view.js` y `views/gantt.view.js`.
- **`limitarFechaSugeridaALimite(fechaSugeridaISO, fechaLimiteISO)`** (v0.74.0): `tarea_fecha_sugerida` nunca puede superar `tarea_fecha_limite` — si las dos están cargadas y la sugerida queda después del día límite, la descarta (devuelve `''`) en vez de bloquear el guardado; la tarea vuelve a ser candidata de `programarTareasSinFecha`. La usan `formulario-tarea.js` (alta/edición individual) y `edicion-masiva.js` (por tarea, contra su propio límite).
- **`mejorTareaPorCategoria(tareas, categorias)`**: ver `REGLAS_DE_PRIORIDAD.md`.
- **`reprogramarFechasSugeridasVencidas(listaTareas)`**: reprograma automáticamente la `tarea_fecha_sugerida` vencida de toda tarea activa a la próxima fecha disponible (`calcularProximaFechaSugerida`, interna, reusa `siguienteDiaHabil` de `reprogramar.js`), en cascada vía `reprogramarTareaConCascada`. Se llama una vez al iniciar la app (`app.js`). Devuelve `{ afectadas, inconsistentes }` (v0.76.0): las tareas afectadas, para avisar al usuario, y las que quedaron con la sugerida después de su límite (ver `avisoInconsistentes`). Ver `REGLAS_DE_PRIORIDAD.md`.
...
UI del panel de reprogramar (usado desde Hoy, Tareas, Agenda y "Revisar mi día").

- **`crearPanelReprogramar({ onConfirmar, onCancelar, diasHabiles })`**: arma un panel con atajos de día y de horario **opcional** (mañana/tarde/tardecita/noche, o vacío). Si la tarea tiene `tarea_dias_habiles`, cualquier fecha elegida se ajusta automáticamente al próximo día hábil. Al confirmar, llama `onConfirmar(valor)` — solo la fecha (`YYYY-MM-DD`) si no se eligió horario, o un datetime ISO completo si sí. También usado por Hoy para revalorizar `tarea_fecha_limite` de una tarea vencida. Tocar un atajo de día lo resalta (clase `.activo`, se apaga en los demás); tocar el campo de fecha a mano apaga el resaltado (v0.76.0).
- **`calcularAtajosDia()`** (privada, v0.76.0 — antes un array fijo `ATAJOS_DIA` con "hoy/mañana/+7/+15/+30 días"): Hoy(+0)/Mañana(+1)/Pasado mañana(+2) y, para los 7 días siguientes (+3 a +9), uno "Próximo <día>" por cada día de la semana real (calculado desde `new Date().getDay()`, siempre los 7 distintos entre sí) — se recalcula en cada apertura del panel porque depende de qué día es hoy.
...
- **`renderPaso()`**: por cada tarea, ofrece "Cumplida" (si tiene `tarea_mantenimiento`, pide una nota de mejora opcional, y llama `cumplirTarea`; después ofrece exportar a Calendar y `ofrecerCrearTareaSeguimiento`, v0.76.0), "No cumplida" (abre directamente el panel de reprogramar, avisando con `avisoInconsistentes` si algún dependiente queda inconsistente) o "Saltar".
...
- **`construirUrlExportarGoogleCalendar(tarea)`**: arma una URL de `calendar.google.com/render` con el evento precargado. El horario es el que STDL tenía planificado (`tarea_fecha_sugerida` + `tarea_duracion_min`, v0.68.0); si la tarea se completó sin haber tenido nunca una fecha sugerida, usa de respaldo `tarea_fecha_fin` menos la duración (comportamiento de antes). El detalle lleva categoría y descripción, si las tiene.
...
- **`renderTarjetaTarea(tarea)`**: arma la tarjeta de una tarea (badges, aviso de bloqueo con la tarea de la que depende, aviso de clima) con un botón "Posponer" que reprograma vía `reprogramarTareaConCascada`, avisando con `avisoInconsistentes` si algún dependiente queda inconsistente (v0.76.0).
...
- **`construirPromptPrioridades(tareas, categorias)` / `parsearRespuestaPrioridades(texto, tareasDisponibles)`**: pide un `tarea_urgente` (booleano, v0.75.0 — antes `tarea_importancia` de 3 valores) sugerido por tarea accionable actual; devuelve solo los cambios reales.
...
- **`listarColoresEvento()`** (v0.68.0): `{ [colorId]: hexDeFondo }` a partir de `GET calendar/v3/colors` (la paleta fija de Google para el color propio de un evento). Cacheada, pero no se invalida junto con `invalidarCacheEventos` (la paleta prácticamente no cambia); `{}` sin conexión o con error.
- **`obtenerEventos(desde, hasta)`** (async): los eventos **que ocupan tiempo** entre dos fechas locales `YYYY-MM-DD`, ambas incluidas, de todos los calendarios elegidos (`pref_calendarios`; sin lista, todos). Cada evento trae `{ id, resumen, inicio, fin, todoElDia, rechazado, disponible, calendarioId, calendarioNombre, color, enlace }` (los de todo el día se pasan a medianoche local); `color` es el propio del evento (`colorId`, vía `listarColoresEvento`) si lo tiene, si no el de su calendario (v0.68.0). Se descartan los que las preferencias mandan ignorar (**`ocupaTiempo`**). Pagina (250 por página), consulta cada calendario por separado (si falla uno, siguen los demás) y cachea los eventos completos 5 minutos por rango y calendarios; los filtros se aplican al leer.
...
- **`async programarTareasSinFecha(estado)`**: recorre las candidatas siguiendo sus cadenas (`tarea_dependiente`; cada una espera a que su previa quede programada, al menos un día después). Para cada una: el día más temprano posible es `max(hoy, tarea_fecha_inicio_habilitada real, día siguiente al de su previa)`; desde ahí busca, día por día, el primero con minutos libres suficientes (`crearCalculadoraCapacidad` de `capacidad.js` — la carga de cada día se acumula aparte, sumando lo que esta misma pasada va asignando, porque la calculadora solo ve una foto de las tareas de antes de empezar) y, dentro de ese día, el primer hueco horario (`buscarHuecoLibre` de `google-calendar.js`) que no choque ni con Calendar (si hay conexión — si no, se programa igual solo con el tope de minutos) ni con otra tarea de STDL ya asignada ese día. La búsqueda de día **corta al llegar al día de `tarea_fecha_limite`** (v0.74.0: la sugerida nunca supera el límite). Si no encuentra hueco dentro del horizonte configurado (o antes de su límite), la deja sin programar (se reintenta en la próxima sesión). Devuelve `{ asignadas, sinHueco }` (v0.76.0): las que sí programó y las que no encontraron hueco (antes quedaban sin sugerida en silencio — ahora `reprogramarSiCorresponde` en `app.js` las nombra en el aviso; en Gantt podían verse con su posición virtual como si algo estuviera roto).
- **`async reubicarTareasSolapadas(estado)`** (v0.74.0): tareas activas con `tarea_fecha_sugerida` con hora que quedó tapada por un evento de Calendar cargado después de asignarla (`calcularSolapamiento` de `google-calendar.js`) — las reubica solas en el próximo hueco libre (`buscarHuecoLibre`), sin pasar de `tarea_fecha_limite` si la tiene. Sin conexión a Calendar no hace nada. Devuelve `{ movidas, sinHueco }`; `sinHueco` son las que chocan pero no tienen hueco libre antes de su límite (quedan como estaban, para que el usuario las revise a mano — `reprogramarSiCorresponde` en `app.js` las nombra en el aviso).
- **`async reprogramarTareaInmediataSiVencio(estado)`** (v0.74.0): la tarea más inmediata (la de `tarea_fecha_sugerida` con hora más próxima entre las activas, sin importar si ya pasó) — mientras "ahora" está dentro de su ventana estimada (`[sugerida, sugerida + tarea_duracion_min]`) no se toca (el usuario todavía no la empezó o la está haciendo ahora mismo); recién cuando esa ventana venció sin completarse busca el próximo hueco real desde ahora (`buscarHuecoLibre`, sin pasar de `tarea_fecha_limite`) y reprograma con `reprogramarTareaConCascada` (`tareas-logica.js`). Se llama una vez al iniciar (dentro de `reprogramarSiCorresponde`) y, mientras la app sigue abierta, cada 1-2 minutos desde un `setInterval` en `app.js` — ahí, si reprogramó algo, se persiste **en silencio** (sin `alert()`, para no ser invasivo cada vez que corre). Devuelve `{ tarea, sinHueco, inconsistentes }` (`inconsistentes` desde v0.76.0, ver `avisoInconsistentes` en `tareas-logica.js`).
- **`async programarParaHoy(tarea, estado)`** (v0.75.0): le asigna a `tarea` un hueco real hoy (respetando Calendar y `tarea_fecha_limite`, mismo patrón de `buscarHuecoLibre`+`superaLimite` que las dos funciones de arriba); si no entra hoy, sigue buscando hacia adelante dentro del horizonte en vez de dejarla sin fecha. La usan el formulario de tarea, la edición masiva y "Reestructurar prioridades con IA" al marcar `tarea_urgente = true` — ver `assets/js/modal-tarea.js`, `assets/js/edicion-masiva.js` y `views/tareas.view.js`. Devuelve `{ tarea, sinHueco, inconsistentes }` (`inconsistentes` desde v0.76.0).
- **`superaLimite(fechaISO, limiteISO)`** (privada): `false` sin límite cargado; con hora en el límite compara el instante exacto, sin hora compara el día calendario. La usan `reubicarTareasSolapadas`, `reprogramarTareaInmediataSiVencio` y `programarParaHoy` para el techo de `tarea_fecha_limite`.
...
- **`renderVistaHoy(contenedor)`**: arma las secciones **Vencidas** (v0.74.0: `esVencida(tarea_fecha_limite)`, sin `tarea_fecha_fin`; solo aparece si hay alguna) / Urgentes (vence hoy **o** `tarea_fecha_sugerida` es hoy — v0.74.0 sumó la sugerida, antes solo era el límite; ya no incluye las vencidas, que se separaron) / Próximos por categoría (`mejorTareaPorCategoria`, con el camino de la categoría) / Resto (sin repetir los próximos; no aparece si no queda nada) / Todavía no pueden empezar / Bloqueadas / Completadas hoy, filtradas por la ubicación actual. Resto/Todavía no pueden empezar/Bloqueadas van en un `<details class="completadas-plegadas">` cerrado por defecto (v0.74.0, mismo patrón que "Completadas" de Tareas), con la cantidad en el título. El botón "🎯 Enfoque" (preferencia `super-todo-list:hoy-enfoque` en `localStorage`, apagado por defecto) oculta las completadas. "Completadas hoy" son las de `tarea_fecha_fin` en el día local de hoy (`seCompletoHoy`). **v0.76.0**: las bloqueadas cuya `tarea_fecha_limite` está vencida/es hoy o cuya `tarea_fecha_sugerida` es hoy (`bloqueadasHoy`) se muestran también en "Urgentes" (de solo lectura, `renderItem(tarea, { soloInfo: true })`) y se sacan de "Bloqueadas por otras tareas" para no duplicarlas; mientras "Urgentes" tenga algo (`urgentes` o `bloqueadasHoy`), "Próximos por categoría" se oculta (no tiene sentido sugerir algo optativo habiendo algo urgente).
...
- **`renderItem(tarea, opciones)`**: tarjeta de una tarea con sus badges (importancia, categoría, fechas, holgura, estado, ubicación, costo, clima ☀️/🌧️), el checklist tildable y el aviso de superposición con Calendar con sus botones "Posponer" y "Al próximo hueco libre" (este usa `obtenerEventosDelHorizonte`, `buscarHuecoLibre`, `obtenerFranjaHoraria` y `reprogramarTareaConCascada`). Si es accionable, agrega los botones "Cumplida"/"No cumplida"; si además está vencida, agrega "📅 Revalorizar fecha límite" (escribe directo `tarea.tarea_fecha_limite` sin cascada a dependientes — ver `REGLAS_DE_PRIORIDAD.md`). No muestra "Desde" (v0.74.0: sacado, no aportaba); las fechas sugerida y límite se muestran con `textoFechaHumana` (`utilidades.js`). "Bloqueada por" solo se muestra si la tarea propia está `tarea_estado === 'bloqueada'` (v0.74.0, bug fix: antes se mostraba con solo tener `tarea_dependiente`, aunque esa previa ya estuviera completada y el bloqueo ya no existiera).
...
- **Eventos y carga** (Ronda 9b): tras dibujar, `pintarCargas` muestra en cada día la barra "planificado/disponible" (con `crearCalculadoraCapacidad`; primero sin eventos y luego con los de Calendar) y `pintarEventos` agrega los eventos de `obtenerEventosParaMostrar` como bloques de solo lectura (enlaces a Calendar, carriles si se pisan) y la franja de los de todo el día. Cada bloque (salvo los "Disponible", que siguen transparentes) lleva de fondo su propio `evento.color` tintado (**`hexARgba(hex, alpha)`**, pura, v0.68.0; `alpha = 0.55` desde v0.76.0, antes 0.22, para que se vea más parecido a Google Calendar), además del borde izquierdo que ya tenía. Tocar la barra abre `abrirCapacidadDelDia` (fija `pref_capacidad_por_fecha`).
...
- **`renderBloqueTarea(...)` / `agregarAsasArrastre(...)`**: cada bloque tiene asas arrastrables (Pointer Events) para modificar `tarea_fecha_sugerida` (agregándole/cambiándole la hora)/`tarea_duracion_min` directamente desde la grilla, en pasos de 15 minutos. Un bloque de 15 minutos o menos suma la clase `.corto` (v0.76.0), con una fuente más chica para que el nombre no se corte.

## `views/tareas.view.js`

La vista más grande: alta de tareas, filtros, lista y el panel de IA.

- **`renderVistaTareas(contenedor)`**: botón "＋ Nueva tarea" (abre `abrirAltaTarea`), los filtros (categoría, estado, ubicación, importancia y buscador por nombre — v0.74.0) y la lista. Orden: pendientes → bloqueadas (por prioridad dentro de cada grupo) y, al final, las completadas plegadas en un `<details class="completadas-plegadas">` "Completadas (N)" que recuerda si estaba abierto (y se muestra abierto con el filtro Estado = Completada). El buscador redibuja con `conservarFoco` (`utilidades.js`, v0.76.0), para no perder el foco en cada letra escrita.
- **`renderTarea(tarea, indice, activas, actualizarBarraSeleccion)`**: tarjeta con el borde izquierdo del color de la categoría (`--color-categoria`), etiqueta "⚠️ Vencida" y fondo rojizo si está vencida, los badges (incluida la persona asociada, 👤, desde la v0.66.0), el checklist con casillas que se tildan ahí mismo (`checklist-tarjeta.js`, persiste al tildar) y las acciones (▲▼ para reordenar a mano — v0.69.0, ver más abajo —, cambiar estado — solo `pendiente`/`completada` —, posponer, editar, duplicar, crearle previa/posterior, eliminar). "Editar" abre `abrirEdicionTarea` (también con doble clic en la tarjeta fuera de modo selección — v0.74.0); completar usa `cumplirTarea`, volver a pendiente `reabrirTarea` y eliminar `eliminarTarea`. `indice`/`activas` (el orden global, aunque "Agrupar por categoría" esté activo) son opcionales: sin ellos (las completadas) no se muestran las ▲▼. En modo selección (v0.70.0) suma una casilla al principio de la tarjeta; tocar la tarjeta fuera de un control real alterna esa casilla (v0.74.0).
- **▲▼ reordenar a mano** (v0.69.0): comparan `tarea` contra `activas[indice - 1]`/`activas[indice + 1]` con `motivoBloqueoOrdenManual` (`tareas-logica.js`); si no se puede, el botón queda `disabled` con el motivo en el `title` (título especial "(reordena la cadena)" si la vecina es cadena directa — v0.75.0). Al click, si la vecina es su previa/próxima directa usa `intercambiarCadena` (v0.75.0); si no, `intercambiarAdyacentes` (v0.74.0) — y `persistirYNotificar()` (el redibujado sale solo, por la suscripción de `app.js`, mismo patrón que ▲▼ de Categorías).
- **`crearPanelIAPrioridades()`**: UI del flujo de copiar/pegar con IA para reestructurar `tarea_urgente` de las tareas accionables; marcar una como urgente al aplicar los cambios también le asigna hoy (`programarParaHoy`, ver `assets/js/programador.js`), avisando con `avisoInconsistentes` (acumulado de todo el lote) si algún dependiente queda inconsistente.
- **Selección múltiple y edición masiva** (v0.70.0): "☑️ Seleccionar" alterna `modoSeleccion` (variable de módulo); activado, cada tarjeta de `activas` suma una casilla (`data-seleccionar`) ligada al `Set` de módulo `seleccionadas`, y aparece "☑️ Seleccionar todas" (v0.74.0, tilda todas las `activas` visibles) en la barra de selección. Tildarla solo actualiza `seleccionadas` y la barra de selección (`actualizarBarraSeleccion`, cuenta + habilita "✏️ Editar tareas seleccionadas") — **sin** redibujar toda la lista, para no perder el scroll. Al entrar a `renderVistaTareas` se filtra `seleccionadas` contra `estado.tareas` vigentes. "✏️ Editar tareas seleccionadas" abre `abrirEdicionMasiva` (v0.74.0: extraída a `assets/js/edicion-masiva.js`, compartida con `views/tabla.view.js` — ver esa sección).

## `assets/js/edicion-masiva.js`

Edición masiva (v0.70.0 en Tareas; extraída a módulo compartido en v0.74.0 para reusarla también en Tabla): aplicar el mismo cambio a varias tareas elegidas de una vez.

- **`abrirEdicionMasiva(tareas, alTerminar)`**: `abrirDialogoFormulario` con `htmlFormularioEdicionMasiva()` — una fila por campo (`filaCampoMasivo`) con una casilla "Cambiar" que habilita el control real; sin tildar, ese campo no se toca. Campos: categoría, urgente (interruptor Sí/No, v0.75.0 — antes un `<select>` de importancia), disfrute, meta, persona, ubicación (desplegables de `formulario-tarea.js`, sin la opción "＋ Crear nueva…", recortada con `sinOpcionNueva`), habilitada desde / fecha sugerida / fecha límite (con hora, v0.74.0 — antes solo la fecha límite, sin hora), duración, costo estimado y días hábiles (`htmlDiasHabiles`). Al guardar, `leerEdicionMasiva` arma `cambios` solo con los campos tildados y `aplicarCamposATarea(tarea, cambios)` (`formulario-tarea.js`) los aplica a cada tarea elegida; sin ningún campo tildado, avisa y no hace nada. Si se tildó "Fecha sugerida", por cada tarea se recorta contra **su propia** fecha límite (`limitarFechaSugeridaALimite` de `tareas-logica.js` — puede ser distinta entre las tareas elegidas, o haber cambiado recién si también se tildó "Fecha límite"); si se recortó en alguna, avisa cuántas. Si se tildó "Urgente" en Sí, cada tarea del lote se agenda para hoy (`programarParaHoy`, `assets/js/programador.js`, v0.75.0), sin evitar chocar entre sí (`reubicarTareasSolapadas` las reacomoda solas después); se acumulan los `inconsistentes` de todo el lote y se avisan en un solo `alert()` al final (`avisoInconsistentes`, v0.76.0). Quedan fuera a propósito: nombre/descripción, los enlaces depende-de/bloquea-a (regla 1 a 1, no se le puede asignar la misma previa a varias tareas) y marcar como completada (dispara efectos por tarea que no encajan con "aplicar lo mismo a todas"). Lo usan `views/tareas.view.js` y `views/tabla.view.js`.
...
- **`htmlFormularioTarea(tarea | null, { modo, botonesNombre, botonesPie })`**: nombre arriba (con autocompletado por nombre solo en el alta) y debajo todos los campos: categoría, disfrute, meta, persona, urgente (interruptor Sí/No — v0.75.0, antes un `<select>` de importancia), los 3 pares fecha+hora, duración, costo, descripción, ubicación, "depende de (tarea previa)" y "bloquea a (tarea próxima)" (`opcionesPrevia`/`opcionesProxima`), clima, mantenimiento con intervalo, desencadenante y checklist editable, y días hábiles. También exporta los helpers de opciones (`htmlOpcionesCategoria`, etc.) y `tareasUnicasPorNombre`.
...
- **`conectarFormularioTarea(formulario, { modo, precargaPorNombre })`**: pone en mayúscula la primera letra del nombre mientras se escribe, muestra u oculta lo de mantenimiento, agrega/quita pasos del checklist (Enter en un paso agrega otro en vez de enviar) y, en el alta con `precargaPorNombre` (default `true`), precarga los demás campos cuando el nombre coincide exacto con una tarea existente, **sin pisar lo que el usuario ya cargó** (solo completa campos que siguen como estaban o que la propia precarga había completado). `precargaPorNombre: false` (v0.66.1, usado cuando `abrirAltaTarea` recibe un `origen`) apaga esto entero: si el formulario ya viene precargado desde otra tarea (Duplicar, Crearle previa/posterior), un nombre que coincida con una tercera tarea no debe pisar esos valores — el campo "sin tocar" se detecta comparando contra el HTML por defecto, que en ese caso ya es el valor de origen, no uno vacío.
...
- **`leerFormularioTarea(formulario)`**: devuelve `{ campos, previaId, proximaId, completada, notaMejora }` (`completada` es `null` si el formulario no trae el interruptor o está deshabilitado); **`aplicarCamposATarea(tarea, campos)`** los aplica a una tarea existente. `campos.tarea_fecha_sugerida` pasa por `limitarFechaSugeridaALimite` (`tareas-logica.js`, v0.74.0): si supera la fecha límite cargada, se guarda vacía en vez de bloquear el guardado.
...
- **`abrirDialogoFormulario({ titulo, cuerpoHtml, textoGuardar, botonesGuardar, conectar, alGuardar, alCerrar })`** (con `botonesGuardar` hay varios botones de guardado; `alGuardar` recibe cuál se apretó y una función `reiniciarFirma`): cada llamada crea su propio `<dialog>` en `document.body` (se pueden apilar) y lo quita al cerrar, sin depender del evento `close`. `alGuardar(formulario)` devuelve `true` para cerrar o `false` para dejarla abierta. Esc, el clic afuera (solo si empezó y terminó afuera — medido por **contención en el DOM**, `dialogo.contains`, no por coordenadas desde la v0.66.1: un popover propio en `position: fixed`, como el selector de color, puede dibujarse visualmente fuera del rectángulo del diálogo y seguir siendo "de adentro") y "Cancelar" preguntan "¿Descartarlos?" solo si el formulario cambió (`firmaFormulario`).
...
- **`abrirDialogoPersona`, ampliado (v0.66.0)**: suma "📅 Próximo contacto" y, solo al editar una persona existente, la lista de sus tareas **pendientes** asociadas (`tareasPendientesDe`, `persona_id` + no completada, ordenadas con `compararPorPrioridad`), cada una con un ✏️ que abre `abrirEdicionTarea` (de `modal-tarea.js`) encima. Si "Próximo contacto" cambió y no quedó vacío, al guardar reprograma con `reprogramarTareaConCascada` la `tarea_fecha_sugerida` de cada una de esas tareas a esa fecha, acumula los `inconsistentes` de todas y avisa cuántas se movieron y, aparte, cuáles quedaron con la sugerida después de su límite (`avisoInconsistentes`, v0.76.0). El campo no permite elegir una fecha pasada (`min` de hoy más una validación al guardar, por si se escribe a mano o se pega un valor).
...
- **`abrirAltaTarea(origen, { previaId, proximaId })`**: ventana "Nueva tarea" con el formulario compartido. "Agregar y cargar otra" va primero en el DOM (lo dispara Enter): agrega, regenera las opciones de "Depende de"/"Bloquea a" (para poder elegir la tarea recién creada) y deja la ventana vacía con el cursor en el nombre; **"➡️ Agregar y crearle siguiente"** (v0.66.1) agrega, cierra esta ventana y abre una nueva en blanco con `previaId` = la tarea recién creada (para cargar una cadena de una sola pasada); "Agregar" agrega y cierra. Con `origen` (v0.64.0) precarga el formulario con esos datos (y apaga la precarga por nombre, ver `conectarFormularioTarea`); `previaId`/`proximaId` fuerzan la selección inicial de esos desplegables. Valida, crea la tarea, aplica los enlaces (un pedido contradictorio no crea la tarea ni limpia el formulario), si queda `bloqueada` sin una "fecha desde" propia hereda la de su previa (v0.66.1), si nace urgente la agenda para hoy (`programarParaHoy`, `assets/js/programador.js`, v0.75.0) y ofrece marcar la cadena de un anillo de mantenimiento. La abren el "＋" de la cabecera, la tecla N y el botón flotante de la vista Tareas.
...
- **`abrirEdicionTarea(id)`** (sobre `abrirDialogoFormulario`): la abre encima de la vista actual (Tareas, Tabla, Gantt o Semana) con el formulario compartido. Esc y el clic afuera pasan por la confirmación "Hay cambios sin guardar. ¿Descartarlos?" solo si el formulario cambió. Botón **"🗑️ Eliminar"** (v0.76.0, vía `botonesGuardar`, a la izquierda de "Guardar cambios"): pide confirmación, llama `eliminarTarea` y cierra sin pasar por la validación/aplicación de campos del resto del flujo. Al guardar (el resto de los botones): si la tarea se eliminó mientras se editaba, avisa y cierra; si su `tarea_modificado_en` cambió desde que se abrió (cambio de otro dispositivo), pide confirmar que se pisan esos cambios; valida el nombre y los enlaces; aplica los campos y `aplicarEnlace`; si `tarea_urgente` pasó de `false` a `true` en esta edición (no si ya era urgente y se re-guardó sin tocar ese campo) la agenda para hoy (`programarParaHoy`, v0.75.0, avisando con `avisoInconsistentes` si algún dependiente queda inconsistente); y persiste.
- **`ofrecerCrearTareaSeguimiento(tarea)`** (v0.76.0): tras completar una tarea, `confirm()` para crear una tarea de seguimiento con los mismos datos (`copiaDeTarea` con `vaciarNombre: true`) pero **sin fechas** (a diferencia de Duplicar/Crearle previa/posterior, acá no tendría sentido arrastrar una fecha límite ya vencida) y sin enlazarla con la recién completada. Se llama junto a `ofrecerExportarACalendar(tarea)` en los mismos puntos donde ya se llama esa (Hoy, Tareas, "Revisar mi día").
...
- **`renderVistaTabla(contenedor)`**: por defecto ordena por `compararPorPrioridad` (el orden real de la app), para detectar de un vistazo si algo quedó mal priorizado. Filtros de categoría (inclusivo de descendientes, vía `idsCategoriaYDescendientes`), estado (**"🚦 Pendientes y bloqueadas" por defecto** desde v0.74.0, antes "Todos"), importancia, **persona** (v0.66.0) y buscador por nombre (redibuja con `conservarFoco`, `utilidades.js`, v0.76.0). Clic en un header de columna cambia el orden a esa columna sola (`COMPARADORES`), con toggle asc/desc y un botón "↺ Prioridad" para volver al orden por defecto. Clic en una fila abre esa tarea en edición en Tareas. Con el orden por defecto activo, una columna "Orden" con ▲▼ (v0.69.0, ver `renderFila`) deja reordenar a mano; desaparece al ordenar por una columna. **Selección múltiple y edición masiva** (v0.74.0, mismo patrón que Tareas): "☑️ Seleccionar" alterna `modoSeleccionTabla`; activado, cada fila no completada suma una casilla (columna aparte, convive con "Orden") ligada a `seleccionadasTabla`, y "✏️ Editar tareas seleccionadas" abre `abrirEdicionMasiva` (`assets/js/edicion-masiva.js`). El encabezado de columnas queda fijo (`position: sticky`) al scrollear dentro de `.tabla-tareas-contenedor` (v0.76.0, que ahora tiene su propio scroll vertical, tope 70vh).
- **`renderFila(tarea, columnas, ordenManual, actualizarBarraSeleccionTabla)`** (v0.69.0): fija `--color-categoria` (v0.71.0, mismo mecanismo que las tarjetas de Tareas — pinta el borde izquierdo de la primera celda vía CSS). `ordenManual` (`{ indice, filas }`, solo con el orden por defecto) agrega la celda "Orden" con ▲▼, mismo mecanismo que `views/tareas.view.js` (`motivoBloqueoOrdenManual` + `intercambiarCadena`/`intercambiarAdyacentes` según si la vecina es cadena directa, `tareas-logica.js`, v0.74.0/v0.75.0); los clics de esos botones usan `evento.stopPropagation()` para no disparar el clic de la fila (que abre la edición) — igual que la casilla de selección (v0.74.0).
- **`COLUMNAS` / `columnasVisibles()` / `todasLasColumnasOrdenadas()` / `abrirSelectorColumnas()`**: las 21 columnas posibles (cada una con su `valor` y su `comparar`, incluida `persona` desde la v0.66.0). La columna `mantenimiento` se etiqueta "Repetición" en la UI desde v0.76.0 (la clave interna y el campo de datos no cambian). La preferencia `super-todo-list:tabla-columnas` (`preferenciaColumnas()`/`guardarPreferenciaColumnas()`) guarda `{ orden, visibles }` (v0.71.0: antes era solo un array plano de claves visibles, en el orden fijo de `COLUMNAS`; ese formato viejo se sigue leyendo, tomándolo como `visibles` con `orden` por defecto). `todasLasColumnasOrdenadas()` aplica `orden` sobre `COLUMNAS` (una columna nueva que `orden` no conozca cae al final); `columnasVisibles()` filtra ese resultado por `visibles`. El diálogo "Columnas" es una lista de filas (casilla + ▲▼) que reordena moviendo los nodos del DOM directamente (sin volver a abrir el diálogo, mismo criterio que ▲▼ de Categorías); al guardar se lee el orden final de las filas y cuáles quedaron tildadas.
...
- **`crearPanelVersus(contenedorVista)`**: panel toggleable (botón "⚔️ Versus") que ofrece de a un par de tareas empatadas (`construirClusteres`/`proximoParVersus`, sobre `esTareaAccionable`) para que el usuario elija cuál prefiere, o las omita; elegir llama `asignarOrdenManual` (`tareas-logica.js`, v0.69.0: antes repetía el cálculo acá; misma función que sigue usando el ▲▼ de Gantt, no ya el de Tareas/Tabla desde la v0.74.0). Ver `REGLAS_DE_PRIORIDAD.md` para el mecanismo completo (por qué "omitir" no asigna nada).
...
- **`renderCategoria(categoria, profundidad, tieneHijas, colapsada, redibujar)`**: tarjeta indentada según su profundidad, con ▸/▾ para colapsar (solo si tiene hijas), ▲/▼ para reordenar entre **hermanos** (mismo `categoria_padre_id`; `intercambiarPrioridad` intercambia `categoria_prioridad`), "➕ Agregar categoría hija" (`abrirDialogoCategoria({ padreIdInicial })`), "📋 Ver tareas" (v0.74.0: `establecerFiltroCategoria` de `views/tareas.view.js` + navega a `#/tareas`, ya filtrado), "Editar" (`abrirDialogoCategoria({ id })`) y eliminar (las hijas se promueven a raíz y las tareas asociadas quedan sin categoría).
...
- **`renderVistaGantt(contenedor)`**: controles (interruptor Plan | Ventana, escala 2 · 4 · 12 semanas, "Hoy", "Agrupar por" y filtros, incluido el buscador — redibuja solo `.gantt-desplazable` vía `redibujar()`, envuelto en `conservarFoco` desde v0.76.0) y la leyenda. Modo, escala y agrupación se guardan en `localStorage` (`super-todo-list:gantt-*`); los filtros viven en variables del módulo, igual que el desplazamiento (que se conserva al redibujar).
...
- **`conectarInteracciones(...)`**: arrastre con umbral de 4 px. Sin mover, abre `abrirEdicionTarea`; en Plan mover la barra llama a `reprogramarTareaConCascada` (conserva la hora), avisa con `avisoInconsistentes` si algún dependiente queda inconsistente (v0.76.0) y avisa si queda antes de su previa; en Ventana los bordes cambian habilitada y límite; "📌" guarda el día estimado como fecha sugerida; ▲▼ (v0.69.0, solo en Plan) reordenan a mano.
- **▲▼ reordenar a mano en Gantt** (v0.69.0, solo modo Plan): cada fila conoce a su vecina anterior/siguiente **del mismo carril** (`filas[i-1]`/`filas[i+1]` en `dibujarGrilla`; un separador de carril entre medio no tiene `.tarea` y corta la adyacencia). `motivoOrdenGantt(fila, vecinaFila)` bloquea primero si el `plan.dia` difiere (es lo único que cambiaría algo visualmente en el Gantt) y si no, delega en `motivoBloqueoOrdenManual` (`tareas-logica.js`). Botones en `.gantt-nombre`, junto al "📌"; sus clics no abren la edición (`evento.stopPropagation()` y excluidos del `closest` del handler de `data-abrir`). Sigue usando `asignarOrdenManual` (no `intercambiarAdyacentes`, a diferencia de Tareas/Tabla desde v0.74.0): la adyacencia ya es acotada por carril y día, un caso más chico que el de las otras dos vistas.
- **Categoría junto al nombre** (v0.74.0): `.gantt-nombre` muestra, además del nombre, un `<span class="gantt-categoria">` chico con el nombre de la categoría (color de texto = color de la categoría); también se suma al `title` de la fila.
...
- **`renderPersona(persona)`**: tarjeta con "Editar" (ver `abrirDialogoPersona`, ampliado en la v0.66.0), "Marcar contacto hoy" y, si tiene `persona_proximo_contacto`, su etiqueta. Doble clic en la tarjeta (fuera de un botón/enlace) también abre editar (v0.76.0, mismo patrón que Tareas). Al eliminar, las tareas que la referenciaban quedan con `persona_id: null` (mismo patrón que categorías/ubicaciones/metas).
