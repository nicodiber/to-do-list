...
import { hoyISO, fechaISOMasDias, formatearFecha, noPuedeEmpezarTodavia, escaparHtml } from '../assets/js/utilidades.js';
...
function renderColumnaDia(fechaDia, hoy) {
  ...
  fijas.forEach((tarea) => {
    ...
    cuerpo.appendChild(renderBloqueTarea(tarea, minutosDesdeInicio, tarea.duracion_estimada_min || 30, false));
  });
  ...
  proyectadas.forEach((tarea) => {
    ...
    cuerpo.appendChild(renderBloqueTarea(tarea, cursorMinutos, duracion, true));
...
function renderBloqueTarea(tarea, minutosDesdeInicio, duracionMin, proyectada) {
